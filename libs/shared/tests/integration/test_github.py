...
def valid_handler():
    return Github(
        ...
        token={"key": 10 * "a280"},
        oauth_consumer_token={"key": None, "secret": None, "refresh_token": None},
...
def generic_valid_handler():
    ...
    return Github(
        ...
        token={"key": 10 * "a280", "refresh_token": 10 * "a180"},
        on_token_refresh=lambda token: token,
    )
...
def valid_but_no_permissions_handler():
    return Github(
        ...
        token={"key": 10 * "a280"},  # ThiagoCodecovTester
    )
...
def integration_installed_handler():
    return Github(
        repo={"name": "example-python"},
        owner={"username": "ThiagoCodecov"},
        token={"key": "v1.testfa2yxpxi79hyhqctum1vscqtaiy2dtv3d23e"},
    )
...
class TestGithubTestCase:
    ...
    async def test_get_pull_request_commits_paginated(self, codecov_vcr):
        handler = Github(
            repo={"name": "y0"},
            owner={"username": "y0-causal-inference"},
            token={"key": 10 * "a128"},
            oauth_consumer_token={"key": None, "secret": None, "refresh_token": None},
        )
    ...
    async def test_get_workflow_run(self, codecov_vcr):
        handler = Github(
            repo={"name": "django"},
            owner={"username": "django"},
            token={"key": "test9zwlbanm8k3m3394ihpwyqk08okirro3l3n0"},
        )
        expected_result = {
