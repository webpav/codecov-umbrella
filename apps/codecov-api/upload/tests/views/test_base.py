...
def test_get_repo(db):
    ...
    generic_class.kwargs = {"repo": "codecov::::the_repo", "service": "github"}
...
def test_get_repo_with_invalid_service(db):
    generic_class = GetterMixin()
    generic_class.kwargs = {"repo": "repo", "service": "wrong service"}
...
def test_get_repo_not_found(db):
    generic_class = GetterMixin()
    generic_class.kwargs = {"repo": "repo", "service": "github"}
    with pytest.raises(ValidationError) as exp:
        generic_class.get_repo()
...
def test_get_commit(db):
    ...
    commit.save()
    generic_class = GetterMixin()
    generic_class.kwargs = {"repo": repository.name, "commit_sha": commit.commitid}
...
def test_get_commit_error(db):
    ...
    repository.save()
    generic_class = GetterMixin()
    generic_class.kwargs = {"repo": repository.name, "commit_sha": "missing_commit"}
    with pytest.raises(ValidationError) as exp:
