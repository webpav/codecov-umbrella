...
def test_regular_upload_cleanup(mock_storage):
...
def test_generates_sliced_upload_cleanups(snapshot):
    ...
    latest = create_upload_cleanup_queries()[-1]
    by_id = Upload.objects.filter(pk__in=[1, 2, 3])

    output = dump_delete_queries(latest) + "\n-----\n\n" + dump_delete_queries(by_id)
    assert output == snapshot("upload.txt")
